# Arquitetura do Agent Service — Belle VetCare CRM

## Visão Geral

O Belle VetCare CRM inclui um **Agentforce Service** projetado para automatizar parte da jornada de atendimento da clínica veterinária.
...
- solicite agendamento e cancelamento de atendimento;
- fale com um atendente quando necessário.

A arquitetura do agent foi desenhada para integrar experiência do usuário, validação de cadastro e automações em Salesforce.

---

# 1. Objetivo do Agentforce
...
A jornada do agent foi estruturada com uma regra principal:

## Regra central de negócio
O usuário precisa realizar o cadastro do tutor e do pet antes de solicitar um agendamento.

Com base nisso, o fluxo do agent funciona da seguinte forma:

- ao selecionar **Agendar consulta**, o agent solicita o CPF do tutor;
...
- ao selecionar **Falar com atendente**, o agent transfere o atendimento para a clínica.
...
1. O usuário entra no menu do agent;
2. Escolhe entre **cadastrar** ou **agendar**;
3. Se escolher agendar, o agent solicita o CPF;
4. O CPF é validado;
...
# 4. Componentes da Arquitetura do Agent
...
## 4.1 Flow Register Owner and Pet

### Objetivo
Criar o cadastro do tutor e pet diretamente pela jornada do agent.
...
A jornada do agent pode ser resumida em três blocos principais:

## Bloco 1 — Entrada no atendimento
O usuário acessa o agent e escolhe uma opção do menu principal.
...
# 6. Papel do Agent no Projeto

O Agent foi incluído no Belle VetCare CRM para demonstrar a capacidade de construir uma experiência digital integrada ao Salesforce, combinando:
...
# 7. Resultado da Arquitetura do Agentforce Service
